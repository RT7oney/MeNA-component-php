...
$tcp_worker->onMessage = function ($connection, $data) {
	...
	if ($data) {
		if (common_check_parameter($data, $parameter)) {
			...
			if (count($row) <= 0) {
				$msg = common_response(10003.403, '未查询到相关信息', array());
			} else {
				// $password = password_hash($data['password'], PASSWORD_DEFAULT);
				$password = md5($data['password']);
				// print_r($password);
				if ($password !== $row['password']) {
					$msg = common_response(10003.404, '密码不正确', array());
				} else {
					if ($row['isdev'] == 1) {
						$msg = common_response(10003.405, '已经是开发者了', array());
					} else {
			...
			$msg = common_response(10003.402, '请求失败，参数不符', array());
		}
	} else {
		$msg = common_response(10003.401, '请求失败，没有参数', array());
	}
